SET TIME ZONE 'UTC';

-- Count downloads per user per year
WITH dpupm AS (
     SELECT
       DATE_PART('year', od.created) AS year,
       settings->'country' AS country,
       SUM(total_records) AS totalRecords,
       COUNT(od.key) AS totalDownloads,
       username
     FROM occurrence_download od, public.user u WHERE od.created_by = u.username
     AND status IN('SUCCEEDED', 'FILE_ERASED') AND username NOT IN ('nagios')
     GROUP BY DATE_PART('year', od.created), country, username
)
-- Count distinct users for each year
SELECT dpupm.year, dpupm.country, SUM(dpupm.totalRecords) AS "totalRecords", SUM(dpupm.totalDownloads) AS "totalDownloads", COUNT(DISTINCT dpupm.username) AS "totalUsers" FROM dpupm
GROUP BY year, country
ORDER BY year DESC;
