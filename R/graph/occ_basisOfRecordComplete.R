...
occ_basisOfRecordComplete <- function(sourceFile, targetDir) {
  ...
  data <- ddply(DF, .(snapshot), summarize, complete=sum(complete), total=sum(total))
  data$incomplete <- data$total - data$complete
  total <- ddply(DF, .(snapshot), summarize, total=sum(total))
  ...
  # Observations
  data <- ddply(DF[DF$basisOfRecord=="Observation",], .(snapshot), summarize, complete=sum(complete), total=sum(total))
  data$incomplete <- data$total - data$complete
  total <- ddply(DF[DF$basisOfRecord=="Observation",], .(snapshot), summarize, total=sum(total))
  data <- data[c("snapshot", "complete", "incomplete")]
  data <- melt(data, id=c("snapshot"))
  colnames(data) <- c("snapshot", "status", "occurrenceCount")
  # rev() here priorities the first one highest
  ...
  # Specimens
  data <- ddply(DF[DF$basisOfRecord=="Specimen",], .(snapshot), summarize, complete=sum(complete), total=sum(total))
  data$incomplete <- data$total - data$complete
  total <- ddply(DF[DF$basisOfRecord=="Specimen",], .(snapshot), summarize, total=sum(total))
  data <- data[c("snapshot", "complete", "incomplete")]
  data <- melt(data, id=c("snapshot"))
  colnames(data) <- c("snapshot", "status", "occurrenceCount")
  # rev() here priorities the first one highest
