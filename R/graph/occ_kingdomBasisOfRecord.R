...
occ_kingdomBasisOfRecord <- function(sourceFile, targetDir) {
  ...
  kingdom <- ddply(DF, .(snapshot,kingdom), summarize, occurrenceCount=sum(occurrenceCount))
  ...
  data <- ddply(DF[DF$kingdom=="Animalia",], .(snapshot,basisOfRecord), summarize, occurrenceCount=sum(occurrenceCount))
  # rev() here priorities the first one highest
  data$basisOfRecord <- factor(data$basisOfRecord, rev(displayOrder))
  total <- ddply(data, .(snapshot), summarize, occurrenceCount=sum(occurrenceCount))
  ...
  data <- ddply(DF[DF$kingdom=="Plantae",], .(snapshot,basisOfRecord), summarize, occurrenceCount=sum(occurrenceCount))
  # rev() here priorities the first one highest
  data$basisOfRecord <- factor(data$basisOfRecord, rev(displayOrder))
  total <- ddply(data, .(snapshot), summarize, occurrenceCount=sum(occurrenceCount))
  if(length(total$snapshot) > 0) {
