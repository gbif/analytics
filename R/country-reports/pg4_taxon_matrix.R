...
flatten <- function(df) {
  ...
  for (taxon in taxa) {
    ...
    singleTaxon[,3] <- ifelse(singleTaxon[,3] == 0, "0 %", ifelse(singleTaxon[,3] > 0, paste("+ ", paste(as.character(singleTaxon[,3]), "%", sep=""), sep=""), paste("- ", paste(as.character(singleTaxon[,3]), "%", sep=""), sep="")))
