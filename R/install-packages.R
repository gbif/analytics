
install.packages("reshape2", repos="http://cran.rstudio.com/") # 1.4.3
install.packages("plyr", repos="http://cran.rstudio.com/") # 1.8.4
install.packages("Rcppr", repos="http://cran.rstudio.com/")
install.packages("dplyr", repos="http://cran.rstudio.com/")

install.packages("ggplot2", repos="http://cran.rstudio.com/") #  2.2.1
install.packages("svglite", repos="http://cran.r-project.org")
install.packages("proto", repos="http://cran.rstudio.com/")
# install.packages("http://cran.r-project.org/src/contrib/Archive/ggplot2/ggplot2_1.0.1.tar.gz", repos=NULL, type="source")

install.packages("whisker", repos="http://cran.rstudio.com/") # 0.3.2
install.packages("ISOcodes", repos="http://cran.rstudio.com/") # 2017.9.27
install.packages("jsonlite", repos="http://cran.r-project.org") # 1.5
install.packages("extrafont", repos="http://cran.r-project.org") # 0.17
install.packages("RGoogleAnalytics", repos="http://cran.r-project.org")
install.packages("httpuv", repos="http://cran.r-project.org")
install.packages("RMySQL", repos="http://cran.r-project.org")
install.packages("RPostgreSQL", repos="http://cran.r-project.org")
install.packages("doParallel", repos="http://cran.r-project.org")
